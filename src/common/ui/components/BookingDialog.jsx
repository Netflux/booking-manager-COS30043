	...
	render() {
		...
		return (
			<Dialog contentClassName="dialog" title={this.state.dialogTitle} autoScrollBodyContent={true} actions={actions} open={this.state.open} onRequestClose={() => this.dismiss()}>
				...
				<TextField id="description" className="form-input" floatingLabelText="Description (Optional)" floatingLabelFixed={true} disabled={this.state.mode === MODE_VIEW} onChange={(event) => this.handleChange(event.target.value, 'bookingDesc')} value={this.state.bookingDesc} /><br />
